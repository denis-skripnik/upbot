...
var account='XXXXX'; //<< сюда вписываем аккаунт робота который будет апвоутить посты по вызову пользователя
var k='XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX'; //<< сюда вписываем приватный постинг ключ робота
...
// если в комментарии прозвучала ключевая фраза (с начала комментария, индекса 0 до 8) идём дальше
			if (data.body.substring(0,account.length+5)==='@'+account+' up!') {
				...
				console.log(data.body.substring(0,account.length+3));
				// функция вычленения точки из аккаунта
